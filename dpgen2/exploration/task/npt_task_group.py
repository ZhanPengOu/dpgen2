...
class ReviseTaskGroup(ExplorationTaskGroup):
    ## when user offer their lmp templte
    ## make tasks by this class
    ...
    def set_plm(
        ...
        """
        Set plumed parameters
        """
        self.plm_temps = temps
        self.stride = stride
        self.plm_set = True
    ...
    def make_plm_task(
        ...
        """
        Make the plumed task group.
        
        Returns
        -------
        task_grp: ExplorationTaskGroup
            The returned lammps+plumed task group. The number of tasks is nconf*nT*nT(plm)*nP.
            nconf is set by `n_sample` parameter of `set_conf`. 
            nT and nP are lengths of the `temps` and `press` parameters of `set_md`.
            nT(plm) are lengths of the "plm_temps" parameters of "set_plm"
        ...
        if not self.md_set:
            raise RuntimeError('MD settings are not set')
        if not self.plm_set:
            raise RuntimeError('plumed settings are not set')
        # clear all existing tasks
        self.clear()
        confs = self._sample_confs()
        for cc,tt,ttp,pp in itertools.product(confs, self.temps, self.plm_temps, self.press):
            self.add_task(self._make_plm_task(cc, tt, ttp, pp))
        return self
    ...
    ## differ from "make_plm_task"，this method mean to produce the lammps+plumed input file
    def _make_plm_task(
        self,
        conf : str,
        tt : float,
